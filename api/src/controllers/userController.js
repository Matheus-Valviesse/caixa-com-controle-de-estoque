...
const userController = {
  create: async (req, res) => {
    ...
    try {
      ...
      const validRoles = ["ADM", "SUBADM", "USER"]; // Array com valores válidos para role

      if (!validRoles.includes(role)) {
        throw new Error("Valor de role inválido."); // Lança um erro se role não estiver no array validRoles
      }

      if (role == "ADM") {
        const validAdm = await User.findOne({ where: { role: "ADM" } });
        console.log(validAdm);
        if (validAdm) throw new Error("Não é possivel criar mais de um ADM");
      }
      ...
      return res
        .status(200)
        .json({ mensagem: `${role} ${user.name} cadastrado com sucesso.` });
    } catch (error) {
  ...
  auth: async (req, res) => {
    ...
    try {
      ...
      if (user.banned != false)
        return res.status(404).json({ erro: "Usuário banido." });
  ...
  userBan: async (req, res) => {
    const { userId: id, role: role, bannedId: bannedId } = req.body;

    try {
      const user = await User.findByPk(id);
      const banUser = await User.findByPk(bannedId);

      switch (true) {
        case !user:
          throw new Error("Usuario precisa estar logcado para ter acesso.");
        case user.role != role:
          throw new Error("Usuario precisa estar logcado para ter acesso.");
        case !(user.role == "ADM" || user.role == "SUBADB"):
          throw new Error("Você não tem permissão a essa página.");
        case !banUser:
          throw new Error("Usuario não encontrado ou não existe.");
        case user.userId == banUser.userId:
          throw new Error("Usuario não pode banir a si mesmo.");
        case banUser.role == "ADM":
          throw new Error("Você não tem privilégio para banir esse usuário.");
        default:
      }

      if (banUser.banned == true) {
        banUser.update({ banned: false });

        return res.status(200).json({
          mensagem: `O usuario ${banUser.name} foi desbanido com sucesso.`,
        });
      }

      if (banUser.banned == false) {
        banUser.update({ banned: true });

        return res.status(200).json({
          mensagem: `O usuario ${banUser.name} foi banido com sucesso.`,
        });
      }
